...
best_Test_acc = 0.0  # best PrivateTest accuracy
...
if opt.resume:
    ...
    checkpoint_path = os.path.join(path, 'Test_model.pth')
    legacy_checkpoint = os.path.join(path, 'Test_model.t7')
    checkpoint_file = checkpoint_path if os.path.exists(checkpoint_path) else legacy_checkpoint
    checkpoint = torch.load(checkpoint_file, map_location=device)

    net.load_state_dict(checkpoint['net'])
    best_Test_acc = checkpoint.get('best_Test_acc', best_Test_acc)
    best_Test_acc_epoch = checkpoint.get('best_Test_acc_epoch', best_Test_acc_epoch)
...
def test(epoch):
    ...
    if Test_acc > best_Test_acc:
        ...
        if not os.path.isdir(path):
            os.mkdir(path)
        torch.save(state, os.path.join(path, 'Test_model.pth'))
        best_Test_acc = Test_acc
        best_Test_acc_epoch = epoch
