...
if opt.resume:
    ...
    checkpoint_path = os.path.join(path, 'PrivateTest_model.pth')
    legacy_checkpoint = os.path.join(path, 'PrivateTest_model.t7')
    checkpoint_file = checkpoint_path if os.path.exists(checkpoint_path) else legacy_checkpoint
    checkpoint = torch.load(checkpoint_file, map_location=device)

    net.load_state_dict(checkpoint['net'])
    best_PublicTest_acc = checkpoint.get('best_PublicTest_acc', best_PublicTest_acc)
    best_PrivateTest_acc = checkpoint.get('best_PrivateTest_acc', best_PrivateTest_acc)
    best_PublicTest_acc_epoch = checkpoint.get('best_PublicTest_acc_epoch', best_PublicTest_acc_epoch)
    best_PrivateTest_acc_epoch = checkpoint.get('best_PrivateTest_acc_epoch', best_PrivateTest_acc_epoch)
